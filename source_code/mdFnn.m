...
for i = 1:maxEmb % embed data
    ...
    if i == 1
        ...
        fnnPerc(i) = 100*sum(fnnTemp)/length(fnnTemp);
        a(i) = fnnPerc(i);
        
        if i>1 && (fnnPerc(i) == fnnPerc(i-1))
            if i ==2
                fnnPerc(i) = 0;
            else
                fnnPerc(i-1) = 0;
            end
        elseif i>1 && ( fnnPerc(i)>fnnPerc(i-1))
            fnnPerc(i-1) = 0;
        end
...
if length(find(fnnPerc>0)) == maxEmb
    ind = find(abs(diff(fnnPerc))<=2,1);
    fnnPerc(ind) = 0;
end
